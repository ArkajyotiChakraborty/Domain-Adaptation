...
#Below functions to perform text preprocessing techniques on the given data. 
...
# Split Function for splitting the X (Cleaned Text's) and y(Label's) with user specified split size.
...
# Glove Embeddings for given vocab
...
def train_epoch(model, opt, criterion, dataloader_gossip, dataloader_politi):
  ...
  for i,  (item1, item2) in enumerate(zip(dataloader_gossip, dataloader_politi)):
      ...
      alpha = 0.5
      loss_politi_label.backward()
      opt.step()
      losses.append(loss_politi_label.item())
      num_corrects = sum((torch.sigmoid(pred_politi_label)>0.5) == y_politi)
      acc = 100.0 * num_corrects/len(y_politi)
      accs.append(acc.item())
      if (i%20 == 0):
          print("Batch " + str(i) + " : training loss = " + str(loss_politi_label.item()) + "; training acc = " + str(acc.item()))
  return losses, accs
...
#Training and Evaluation experimentation function
