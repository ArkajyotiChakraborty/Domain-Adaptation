# -*- coding: utf-8 -*-
"""Copy DA-baseline-lstm.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1ZIMfqtRhFRczRZGJWmxA6kGTzuGu1xSz
"""
...
df_gossip = pd.read_csv('/content/drive/MyDrive/Fake News /gossip_train.csv')
df_gossip

df_politi = pd.read_csv('/content/drive/MyDrive/Fake News /politi_train.csv')
df_politi

frames = [df_politi] * 27
df_politi = pd.concat(frames)
df_politi = df_politi.sample(frac = 1)
df_politi
...
df_gossip['article_clean'] = df_gossip['content'].progress_apply(lambda x : preprocessing(x))
df_gossip # For Testing

df_politi['article_clean'] = df_politi['content'].progress_apply(lambda x : preprocessing(x))
df_politi # For Training
...
splits = (0.8, 0.1)
(train_gossip_x, train_gossip_y), (valid_gossip_x, valid_gossip_y), (test_gossip_x, test_gossip_y) = get_splits(df_gossip['article_clean'], df_gossip['label'], splits)
#(len(train_x), len(train_y)), (len(valid_x), len(valid_y)), (len(test_x), len(test_y))

splits = (0.8, 0.1)
(train_politi_x, train_politi_y), (valid_politi_x, valid_politi_y), (test_politi_x, test_politi_y) = get_splits(df_politi['article_clean'], df_politi['label'], splits)
#(len(train_x), len(train_y)), (len(valid_x), len(valid_y)), (len(test_x), len(test_y))
...
class LSTMModel(nn.Module):
    ...
    def __init__(self, embedding_dim, vocabulary_size, hidden_dim, embeddings=None, fine_tuning=False):
        ...
        #self.linear_2 = nn.Linear(in_features=hidden_dim/2, out_features=hidden_dim/4)
        #self.linear_3 = nn.Linear(in_features=hidden_dim/4, out_features=1)
    ...
    def forward(self, inputs_a, inputs_b):
      ...
      x_b_label = torch.squeeze(self.linear_1(h_b)) # task classifier. classification task perfom
...
def train_epoch(model, opt, criterion, dataloader_gossip, dataloader_politi):
  ...
  for i,  (item1, item2) in enumerate(zip(dataloader_gossip, dataloader_politi)):
      x_gossip, y_gossip = item1 # gossip as 0
      x_politi, y_politi = item2 # politi as 1
      opt.zero_grad()
      # Forward pass
      ...
      #loss_politi = criterion(pred_b, y)
      # Backward pass
      loss_total.backward()
...
def eval_model(model, criterion, evalloader_gossip, evalloader_politi):
  ...
  with torch.no_grad():
      for i, (item1, item2) in enumerate(zip(evalloader_gossip, evalloader_politi)):
          ...
          #loss = criterion(pred_politi_label, y_politi)
          num_corrects = sum((torch.sigmoid(pred_politi_label)>0.5) == y_politi)
          acc = 100.0 * num_corrects/len(y_politi)
...
def experiment(model, opt, criterion, num_epochs = 5):
  ...
  for e in range(num_epochs):
      ...
      #valid_loss, valid_acc, val_preds = eval_model(model, criterion, valid_dataloader_politi)
      ...
      print("Epoch " + str(e+1) + " : Validation loss = " + "; Validation acc = " + str(valid_acc))
      print("Epoch " + str(e+1) + " :Gossip Validation loss = " + "; Gossip Validation acc = " + str(valid_acc_gossip))
  ...
  print("Test loss = " + "; Test acc politi = " + str(test_acc_politi) + "; Test Accuracy gossip" + str(test_acc_gossip))
...
cm = confusion_matrix(preds,test_politi_y)
...
print(classification_report(test_politi_y, preds, target_names = ['Predicted Fake','Predicted True']))